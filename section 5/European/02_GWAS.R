...
#####################################################################
#####################################################################
##### Step 5: GWAS for the binary Hypertension traits
#####################################################################
#####################################################################
geno_path = "/home/ziang/Documents/UKB/"
data_path = "/home/ziang/Documents/Stats-Gene/UKB/European_GWAS_hypertension_bp/data/"
plink = "/home/ziang/Documents/UKB/plink2"
plink19 = "/home/ziang/Documents/UKB/plink19"
result_path = "/home/ziang/Documents/Stats-Gene/UKB/European_GWAS_hypertension_bp/results/"
figure_path = "/home/ziang/Documents/Stats-Gene/UKB/European_GWAS_hypertension_bp/figures/"
load("/home/ziang/Documents/Stats-Gene/UKB/European_GWAS_hypertension_bp/data/pheno_data.rda")
...
write.table(x = final_data, file = "/home/ziang/Documents/Stats-Gene/UKB/European_GWAS_hypertension_bp/data/pheno.txt", row.names = F, 
...
system(paste0(plink,
              ...
              " --covar-name age,sex",
              " --glm",
              " --out ", result_path, "self_European_result_hyper"
))
...
#####################################################################
#####################################################################
##### Step 6: GWAS for the continuous blood pressure traits
#####################################################################
#####################################################################
system(paste0(plink,
              " --bfile ", geno_path,"self_European_filtered_data_afterSNP_QC",
              ...
              " --covar-name age,sex",
              " --glm",
              " --out ", result_path, "self_European_result_bp"
))
...
#####################################################################
#####################################################################
##### Step 7: GWAS results summary
#####################################################################
#####################################################################
result_bp <- fread(file = paste0(result_path,"self_European_result_bp.bp.glm.linear"))
result_hyper <- fread(file = paste0(result_path,"self_European_result_hyper.hypertension.glm.logistic"))
...
result_hyper_G$P <- as.numeric(result_hyper_G$P)
...
result_bp_G$P <- as.numeric(result_bp_G$P)
...
#####################################################################
#####################################################################
##### Step 8: Extract the Top 5 SNPs (non-significant) in each trait
#####################################################################
#####################################################################
top_result_hyper <- result_hyper_G %>% filter(P >= 5e-8) %>% arrange(by = P) %>% head(n = 5)
top_result_bp <- result_bp_G %>% filter(P >= 5e-8) %>% arrange(by = P) %>% head(n = 5)
...
#####################################################################
#####################################################################
##### Step 9: Extract the results of Gamma_age, Gamma_sex
#####################################################################
#####################################################################
system(paste0(plink,
              " --bfile ", geno_path,"self_European_filtered_data_afterSNP_QC",
              " --snps ", paste0(c(top_result_bp_summary$SNP, top_result_hyper$SNP), collapse = ", "),
              " --allow-no-sex ",
              " --pheno ", data_path ,"pheno.txt",
              " --pheno-name age",
              " --glm",
              " --out ", result_path, "self_European_gamma_age_result"
))
system(paste0(plink,
              " --bfile ", geno_path,"self_European_filtered_data_afterSNP_QC",
              " --snps ", paste0(c(top_result_bp_summary$SNP, top_result_hyper$SNP), collapse = ", "),
              ...
              " --pheno-name sex",
              " --glm",
              " --out ", result_path, "self_European_gamma_sex_result"
))
result_gamma_age <- fread(file = paste0(result_path,"self_European_gamma_age_result.age.glm.linear"))
result_gamma_age$Gamma_Age <- result_gamma_age$BETA
result_gamma_age$P <- as.numeric(result_gamma_age$P)
result_gamma_sex <- fread(file = paste0(result_path,"self_European_gamma_sex_result.sex.glm.logistic"))
result_gamma_sex$Gamma_Sex <- log(result_gamma_sex$OR)
result_gamma_sex$P <- as.numeric(result_gamma_sex$P)
